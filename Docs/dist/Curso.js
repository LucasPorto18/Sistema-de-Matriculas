import { criarDisciplina } from "./Disciplina.js";
import { alunos, notificarSistemaDeCobrancas } from "./Aluno.js";
const cursos = [];
...
function carregarCursosDoLocalStorage() {
    ...
    if (cursosSalvos) {
        ...
        cursosCarregados.forEach((curso) => cursos.push(curso));
...
function matriculaAluno(Nomealuno, nomeCurso, nomeDisciplina) {
...
function mostrarDisciplinasAluno(nomeAluno) {
    const aluno = alunos.find(a => a.nome === nomeAluno);
    if (!aluno) {
...
function cancelarDisciplinaAluno(nomeAluno, nomeCurso, nomeDisciplina) {
    if (!periodoDeMatriculas) {
        return "Período de matrículas está fechado.";
    }
    const curso = cursos.find(c => c.nome === nomeCurso);
...
function criarCurso(nome, creditos) {
    const novoCurso = {
        nome: nome,
        creditos: creditos,
        disciplinas: []
...
function addDisciplina(NomeCurso, NomeDisciplina, numCreditos, obrigatoria) {
    const curso = cursos.find(c => c.nome === NomeCurso);
    if (!curso) {
...
function verAlunosDisciplina(nomeCurso, nomeDisciplina) {
    const curso = cursos.find(c => c.nome === nomeCurso);
    if (!curso) {
        alert("Curso não encontrado!");
        return;
